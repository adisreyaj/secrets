...
async function loadClient(flags) {
    ...
    const baseUrl = flags.baseUrl ?? process.env.SECRETS_API_BASE_URL ?? config?.apiBaseUrl ?? DEFAULT_BASE_URL;
...
async function exportEnv(args) {
    ...
    const output = await apiFetch(baseUrl, token, `/environments/${envId}/export?format=dotenv`);
    if (flags.out) {
...
async function listSecrets(args) {
    ...
    const { client, baseUrl, token } = await loadClient(flags);
    const envId = await client.resolveEnvironmentId();
    const secrets = await apiFetch(baseUrl, token, `/environments/${envId}/secrets?includeValues=true`);
